...
def time_objective(trial):
    ...
    train = np.asarray(samp_trajs[:75,:,:])
    val = np.asarray(samp_trajs[75:,:,:])
    
    train_wei = np.asarray(W_train[:75,:,:])
    val_wei = np.asarray(W_train[75:,:,:])
    
    static_train = np.asarray(static[:75,:])
    static_test = np.asarray(static[75:,:])
    
    staticdata_onehot_train = np.asarray(staticdata_onehot[:75,:])
    staticdata_onehot_test = np.asarray(staticdata_onehot[75:,:])
    
    static_true_miss_mask_train = np.asarray(static_true_miss_mask[:75,:])
    static_true_miss_mask_test = np.asarray(static_true_miss_mask[75:,:])
...
if __name__ == "__main__":
    ...
    samp_trajs = samp_trajs[0:100,tuple(index),:]
    
    W_train = W_train[0:100,tuple(index),:]
