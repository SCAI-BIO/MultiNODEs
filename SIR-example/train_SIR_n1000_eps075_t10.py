...
if __name__ == "__main__":
    ...
    varSIR = pd.read_csv(mypath + '/SIR_n1000_eps075.csv',
                       sep=',',header=0, 
    ...
    #FrozenTrial(number=101, value=9.40336, datetime_start=datetime.datetime(2021, 5, 25, 23, 32, 59), datetime_complete=datetime.datetime(2021, 5, 25, 23, 52, 9), params={'activation_dec': 'none', 'activation_ode': 'tanh', 'activation_rnn': 'tanh', 'batchsize': 0.99479, 'dropout_dec': 0.0178918, 'enctype': 'RNN', 'latent_dim': 1.21916, 'lr': 0.00417377, 'nhidden': 3.1724, 'num_odelayers': 4, 'rnn_nhidden_dec': 0.277881, 'rnn_nhidden_enc': 0.502702, 'scaling_ELBO': 0.933758, 's_dim_static': 2, 'z_dim_static': 3}, distributions={'activation_dec': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'activation_ode': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'activation_rnn': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'batchsize': UniformDistribution(high=1, low=0), 'dropout_dec': UniformDistribution(high=1, low=0), 'enctype': CategoricalDistribution(choices=('LSTM', 'RNN')), 'latent_dim': UniformDistribution(high=4, low=0), 'lr': UniformDistribution(high=0.01, low=0.0001), 'nhidden': UniformDistribution(high=4, low=0), 'num_odelayers': IntUniformDistribution(high=6, low=1, step=1), 'rnn_nhidden_dec': UniformDistribution(high=1, low=0), 'rnn_nhidden_enc': UniformDistribution(high=1, low=0), 'scaling_ELBO': UniformDistribution(high=2, low=0), 's_dim_static': IntUniformDistribution(high=6, low=1, step=1), 'z_dim_static': IntUniformDistribution(high=6, low=1, step=1)}, user_attrs={'nepochs': 208}, system_attrs={}, intermediate_values={}, trial_id=102, state=TrialState.COMPLETE)
    activation_dec = 'none'
    activation_ode = 'tanh'
    activation_rnn = 'tanh'
    batchsize = (0.99479*750)/1000
    dropout_dec = 0.0178918
    enctype = 'RNN'
    latent_dim = 1.21916
    lr = 0.00417377
    nhidden = 3.1724
    num_odelayers = 4
    rnn_nhidden_dec = 0.277881
    rnn_nhidden_enc = 0.502702
    scaling_ELBO = 0.933758
    s_dim_static = 2
    z_dim_static = 3
    nepochs = 208
    ...
    torch.save({'Recon_values': xs_pos_rec,
                ...
                'W_train': W_train}
               ,train_dir + '/Reconstruction_n3000_eps075_t10_normed.pth')
    ...
    torch.save({'Generation_values': xs_pos_posterior,
                ...
                'varnamesstatic':varnamesstatic}
               ,train_dir + '/Generationposterior_n3000_eps075_t10_normed.pth')
