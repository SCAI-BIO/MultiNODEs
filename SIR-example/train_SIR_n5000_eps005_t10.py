...
if __name__ == "__main__":
    ...
    mypath = '/home/pwendland/Dokumente/GitHub/masterarbeit-philipp/NeuralODE_code/Analyse/SIR/SIR_synth_new'
    train_dir = mypath
    #------ Raw Data -------
    
    varSIR = pd.read_csv(mypath + '/SIR_n5000_eps005.csv',
                       sep=',',header=0, 
    ...
    samp_trajs = samp_trajs[0:5000,tuple(index),:]
    
    W_train = W_train[0:5000,tuple(index),:]
    ...
    nepochs=1513
    #FrozenTrial(number=737, value=0.041862, datetime_start=datetime.datetime(2021, 2, 17, 9, 55, 41), datetime_complete=datetime.datetime(2021, 2, 17, 10, 20, 22), params={'activation_dec': 'none', 'activation_ode': 'tanh', 'activation_rnn': 'none', 'batchsize': 0.17319, 'dropout_dec': 0.0515771, 'enctype': 'RNN', 'latent_dim': 0.196854, 'lr': 0.00998745, 'nhidden': 1.59344, 'num_odelayers': 1, 'rnn_nhidden_dec': 0.904631, 'rnn_nhidden_enc': 0.226659, 'scaling_ELBO': 1.35137, 's_dim_static': 6, 'z_dim_static': 2}
    
    activation_dec = 'none'
    activation_ode = 'tanh'
    activation_rnn = 'none'
    batchsize = (0.17319*3750)/5000
    dropout_dec = 0.0515771
    enctype = 'RNN'
    latent_dim = 0.196854
    lr = 0.00998745
    nhidden = 1.59344
    num_odelayers = 1
    rnn_nhidden_dec = 0.904631
    rnn_nhidden_enc = 0.226659
    scaling_ELBO = 1.35137
    s_dim_static = 6
    z_dim_static = 2
    ...
    torch.save({'Recon_values': xs_pos_rec,
                ...
                'W_train': W_train}
               ,train_dir + '/Reconstruction_n7000_eps005_t10_normed.pth')
    ...
    torch.save({'Generation_values': xs_pos_posterior,
                ...
                'varnamesstatic':varnamesstatic}
               ,train_dir + '/Generationposterior_n7000_eps005_t10_normed.pth')
