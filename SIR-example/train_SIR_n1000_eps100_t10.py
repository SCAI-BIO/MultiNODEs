...
if __name__ == "__main__":
    ...
    varSIR = pd.read_csv(mypath + '/SIR_n1000_eps100.csv',
                       sep=',',header=0, 
    ...
    #FrozenTrial(number=106, value=13.6667, datetime_start=datetime.datetime(2021, 5, 26, 3, 57, 11), datetime_complete=datetime.datetime(2021, 5, 26, 4, 32, 52), params={'activation_dec': 'relu', 'activation_ode': 'none', 'activation_rnn': 'relu', 'batchsize': 0.199226, 'dropout_dec': 0.061786, 'enctype': 'RNN', 'latent_dim': 3.05187, 'lr': 0.00123073, 'nhidden': 2.7973, 'num_odelayers': 2, 'rnn_nhidden_dec': 0.95484, 'rnn_nhidden_enc': 0.79226, 'scaling_ELBO': 1.20909, 's_dim_static': 6, 'z_dim_static': 5}, distributions={'activation_dec': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'activation_ode': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'activation_rnn': CategoricalDistribution(choices=('tanh', 'relu', 'none')), 'batchsize': UniformDistribution(high=1, low=0), 'dropout_dec': UniformDistribution(high=1, low=0), 'enctype': CategoricalDistribution(choices=('LSTM', 'RNN')), 'latent_dim': UniformDistribution(high=4, low=0), 'lr': UniformDistribution(high=0.01, low=0.0001), 'nhidden': UniformDistribution(high=4, low=0), 'num_odelayers': IntUniformDistribution(high=6, low=1, step=1), 'rnn_nhidden_dec': UniformDistribution(high=1, low=0), 'rnn_nhidden_enc': UniformDistribution(high=1, low=0), 'scaling_ELBO': UniformDistribution(high=2, low=0), 's_dim_static': IntUniformDistribution(high=6, low=1, step=1), 'z_dim_static': IntUniformDistribution(high=6, low=1, step=1)}, user_attrs={'nepochs': 508}
    activation_dec = 'relu'
    activation_ode = 'none'
    activation_rnn = 'relu'
    batchsize = (0.199226*750)/1000
    dropout_dec = 0.061786
    enctype = 'RNN'
    latent_dim = 3.05187
    lr = 0.00123073
    nhidden = 2.7973
    num_odelayers = 2
    rnn_nhidden_dec = 0.95484
    rnn_nhidden_enc = 0.79226
    scaling_ELBO = 1.20909
    s_dim_static = 6
    z_dim_static = 5
    nepochs = 508
    ...
    torch.save({'Recon_values': xs_pos_rec,
                ...
                'W_train': W_train}
               ,train_dir + '/Reconstruction_n3000_eps100_t10_normed.pth')
    ...
    torch.save({'Generation_values': xs_pos_posterior,
                ...
                'varnamesstatic':varnamesstatic}
               ,train_dir + '/Generationposterior_n3000_eps100_t10_normed.pth')
